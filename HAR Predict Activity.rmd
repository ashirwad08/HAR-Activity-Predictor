...
In order to get an idea of the spatial orientation features with the outcome, let's look at a scatterplot of the Total Measurements with the Classe outcome.  

```{r viz1, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE}
library(GGally)
...
We see that each of the total measurements have small correlations to the Classe outcome. For e.g. while the __total_accel_belt__ measurement seems to have a lot of data indicating classe A, it's not so well correlated with the other classes.  
...
We see that an expected accuracy of `r max(rf.fit$results[,2])*100`% from the random forest algorithm is reasonable. This gives us an Out of Sample error rate that we can expect: `r  1-max(rf.fit$results[,2])`%.
...
We see that an expected accuracy of `r max(svm.fit$results[,2])*100`% from the SVM is reasonable. This gives us an Out of Sample error rate that we can expect: `r  1-max(svm.fit$results[,2])`%. 
...
We see that the Random Forest algorithm with a `r confusionMatrix(pred.rf, cv$classe)$overall[1]*100`% accuracy on the test set causes the *least* amount of Out of Sample error, when compared to SVM and Recursive Partitioning. We therefore apply the Random Forest model to the course test examples. Final model:  
```{r echo=FALSE}
 rf.fit$finalModel
```  
